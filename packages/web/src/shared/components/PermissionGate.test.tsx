...
describe('PermissionGate', () => {
  it('renders children when user has permission', () => {
    ...
    render(
      <PermissionGate projectId="p-1" toolId="TASK" minLevel="STANDARD">
        <button type="button">Tạo công việc</button>
      </PermissionGate>,
    )
  ...
  it('renders fallback when permission is missing', () => {
    ...
    render(
      <PermissionGate projectId="p-1" toolId="TASK" minLevel="ADMIN" fallback={<span>Không có quyền</span>}>
        <button type="button">Xóa công việc</button>
      </PermissionGate>,
    )

    expect(screen.queryByRole('button', { name: 'Xóa công việc' })).toBeNull()
