...
export function ProjectListPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
        ...
        <div className="space-y-3">
          {(data?.projects ?? []).length === 0 ? (
            <EmptyState
              ...
              description={
                canCreateProject
                  ? 'Nhấn "Tạo dự án" để khởi tạo dự án đầu tiên.'
                  : 'Liên hệ quản trị viên để được phân quyền vào dự án.'
              }
            />
          ) : (
      ...
      {showCreate && <CreateProjectModal onClose={() => setShowCreate(false)} onSuccess={() => setShowCreate(false)} />}
    </div>
  )
}
