...
export const projectSettingsApi = {
  async getSettings(projectId: string) {
    const response = await api.get<ApiSingleResponse<ProjectSettingsSummary>>(`/projects/${projectId}/settings`)
  ...
  async removePermissionOverride(params: { projectId: string; userId: string; toolId: ToolId }) {
  ...
  async assignSpecialPrivilege(params: { projectId: string; userId: string; privilege: SpecialPrivilege }) {
