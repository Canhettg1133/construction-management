    ...
    <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-4 transition-colors hover:bg-slate-50">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          >
            Bao cao ngay {new Date(report.reportDate).toLocaleDateString("vi-VN")}
          </span>
          <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-medium text-amber-600">Cho duyet</span>
        </div>
        <p className="mt-0.5 text-xs text-slate-500">{report.project.name}</p>
        <p className="mt-0.5 text-xs text-slate-500">
          Progress: <span className="font-medium">{report.progress}%</span> · {report.workerCount} cong nhan
        </p>
...
export function ApprovalsPage() {
  ...
  const approveReportMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast({ type: "success", title: "Da duyet thanh cong" });
    },
    onError: (error: unknown) => {
      showToast({
        type: "error",
        title: "Loi",
        description: error instanceof Error ? error.message : "Thao tac that bai",
      });
    },
  });
  ...
  const rejectReportMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setRejectReason("");
      showToast({ type: "success", title: "Da tu choi" });
    },
    onError: (error: unknown) => {
      showToast({
        type: "error",
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Duyet cong viec</h1>
          <p className="mt-0.5 text-sm text-slate-500">Danh sach bao cao va task cho duyet</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {reports.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white py-12 text-center">
              ...
              <p className="mt-3 font-medium text-slate-500">Khong co bao cao nao cho duyet</p>
            </div>
          ) : (
            reports.map((report) => (
              <ReportRow
      ...
      {tab === "tasks" && (
        <div className="space-y-3">
          {tasks.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white py-12 text-center">
              <ShieldAlert className="h-12 w-12 text-slate-200" />
              <p className="mt-3 font-medium text-slate-500">Khong co task nao cho duyet</p>
            </div>
          ) : (
            tasks.map((task) => (
              <TaskRow
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <h2 className="text-base font-semibold text-slate-900">Tu choi</h2>
            ...
            <div className="space-y-4 p-5">
              <div>
                <label className="form-label">Ly do tu choi</label>
                <textarea
                  value={rejectReason}
