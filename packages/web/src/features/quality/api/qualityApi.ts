...
export const qualityApi = {
  async list(projectId: string) {
    const response = await api.get<ApiSingleResponse<QualityDashboardResponse>>(
  ...
  async getById(projectId: string, reportId: string) {
    const response = await api.get<ApiSingleResponse<QualityReport>>(
  ...
  async update(
    projectId: string,
    reportId: string,
    payload: Partial<QualityReportPayload> & { inspectorId?: string }
  ) {
  ...
  async sign(projectId: string, reportId: string) {
    const response = await api.post<ApiSingleResponse<QualityReport>>(
      `/projects/${projectId}/quality/${reportId}/sign`
    );
    return response.data.data;
  },
};
