...
function uniqueRoles(roles: Array<ProjectRole | null | undefined>): ProjectRole[] {
  return Array.from(new Set(roles.filter((role): role is ProjectRole => typeof role === 'string' && role.length > 0)))
}
...
export function useDashboardRole(): DashboardRoleContext {
  ...
  return useMemo(() => {
    ...
    const permissionProjectRoles = Object.values(projectPermissions).map((permission) => permission.projectRole)
