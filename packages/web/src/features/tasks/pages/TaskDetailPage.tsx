...
import { z } from 'zod'
import { Edit2, Trash2, MessageSquare, Send, X, AlertTriangle, CheckCircle, XCircle, ShieldAlert } from 'lucide-react'
import {
  getTask,
  updateTask,
  updateTaskStatus,
...
const TASK_STATUS_OPTIONS: Array<{ value: TaskStatus; label: string; color: string }> = [
  { value: 'TO_DO', label: 'Chưa làm', color: 'bg-slate-100 text-slate-600' },
...
function EditTaskModal({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      showToast({ type: 'success', title: 'Đã cập nhật công việc' })
      onSuccess()
    },
    onError: (e: unknown) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <h2 className="text-base font-semibold text-slate-900">Chỉnh sửa công việc</h2>
...
export function TaskDetailPage() {
  ...
  const canEdit =
    canUseTaskStandard &&
    Boolean(task && (task.createdBy === currentUser?.id || task.assignedTo === currentUser?.id || canUseTaskAdmin))
  const canChangeStatus =
    canUseTaskStandard && Boolean(task && (task.assignedTo === currentUser?.id || canUseTaskAdmin))
  const canSubmitForApproval =
    canUseTaskStandard && Boolean(task && task.assignedTo === currentUser?.id && !task.submittedAt)
  ...
  const statusMutation = useMutation({
    ...
    onMutate: async (status) => {
      ...
      queryClient.setQueryData<Task>(detailKey, (current) => (current ? { ...current, status } : current))
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast({ type: 'success', title: 'Đã xóa công việc' })
  ...
  const addCommentMutation = useMutation({
    mutationFn: (content: string) => createTaskComment(currentProjectId, String(taskId), content),
    onSuccess: () => {
      setCommentText('')
      refetchComments()
    ...
    onError: (e: unknown) => {
      showToast({
        type: 'error',
        title: 'Lỗi',
        description: e instanceof Error ? e.message : 'Không thể thêm bình luận',
      })
    },
  })

  const deleteCommentMutation = useMutation({
    mutationFn: (commentId: string) => deleteTaskComment(currentProjectId, String(taskId), commentId),
    onSuccess: () => {
      refetchComments()
    ...
    onError: (e: unknown) => {
      showToast({
        type: 'error',
        title: 'Lỗi',
        description: e instanceof Error ? e.message : 'Không thể xóa bình luận',
      })
    },
  })
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['tasks', currentProjectId] })
      showToast({ type: 'success', title: 'Đã gửi duyệt công việc' })
    },
    onError: (e: unknown) => {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: (updated) => {
      queryClient.setQueryData(['task', currentProjectId, taskId], updated)
      queryClient.invalidateQueries({ queryKey: ['tasks', currentProjectId] })
      showToast({ type: 'success', title: 'Đã duyệt công việc' })
    },
    onError: (e: unknown) => {
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      setRejectReason('')
      showToast({ type: 'success', title: 'Đã từ chối công việc' })
    },
    onError: (e: unknown) => {
  ...
  if (isError || !task) {
    return <ErrorState message="Không tải được chi tiết công việc. Vui lòng thử lại." />
  }
  ...
  const canReviewTask =
    isPmOrAdmin && task.requiresApproval && task.approvalStatus === 'PENDING' && Boolean(task.submittedAt)
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {task.assignee && <p className="text-xs text-slate-500">Giao cho: {task.assignee.name}</p>}
        </div>
        <PermissionGate projectId={currentProjectId} toolId="TASK" minLevel="STANDARD">
          <div className="flex items-center gap-1">
            {canSubmitForApproval && task.requiresApproval && task.approvalStatus === 'PENDING' && (
              <button
            ...
            {canUseTaskAdmin && (
              <button
                onClick={() => {
                  if (confirm('Xóa công việc này?')) deleteMutation.mutate()
                }}
        ...
        <div className="grid grid-cols-2 gap-3">
          {currentPriority && (
            <div className="rounded-xl bg-slate-50 p-3">
              ...
              <span className={`text-sm font-semibold ${currentPriority.color}`}>{currentPriority.label}</span>
            </div>
          )}
          <div className={`rounded-xl p-3 ${isOverdue ? 'bg-red-50' : 'bg-slate-50'}`}>
            <span className="text-xs text-slate-500">Hạn chót: </span>
          ...
          <div className="rounded-xl bg-slate-50 p-3">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <approvalConfig.icon
                  className={`h-4 w-4 ${approvalConfig.color.replace('bg-', 'text-').replace(' text-', '')}`}
                />
      ...
      <div className="app-card space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-slate-700">Bình luận ({comments.length})</h3>
        </div>
          ...
          <div className="space-y-2">
            {comments.map((comment) => (
              <div key={comment.id} className="flex gap-3 rounded-xl bg-slate-50 p-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-medium text-slate-900">{comment.author?.name ?? 'Người dùng'}</span>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <h2 className="text-base font-semibold text-slate-900">Từ chối công việc</h2>
              <button
