...
export const safetyApi = {
  async list(projectId: string) {
    const response = await api.get<ApiSingleResponse<SafetyDashboardResponse>>(
  ...
  async getById(projectId: string, reportId: string) {
    const response = await api.get<ApiSingleResponse<SafetyReport>>(
  ...
  async update(
    projectId: string,
    reportId: string,
    payload: Partial<SafetyReportPayload> & { inspectorId?: string }
  ) {
  ...
  async sign(projectId: string, reportId: string) {
    const response = await api.post<ApiSingleResponse<SafetyReport>>(
      `/projects/${projectId}/safety/${reportId}/sign`
    );
    return response.data.data;
  },
};
