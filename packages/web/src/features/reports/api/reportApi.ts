...
export async function listReports(projectId: string, params?: {
  page?: number;
  pageSize?: number;
  from?: string;
  to?: string;
...
export async function createReport(projectId: string, payload: {
  reportDate: string;
...
export async function updateReportStatus(projectId: string, reportId: string, status: "DRAFT" | "SENT"): Promise<DailyReport> {
...
export async function uploadReportImages(projectId: string, reportId: string, files: File[]): Promise<ReportImage[]> {
  ...
  const res = await api.post<ImageArrayResponse>(
    `/projects/${projectId}/reports/${reportId}/images`,
    formData,
