...
function getEncryptionKey() {
  if (!env.AI_SECRET_ENCRYPTION_KEY?.trim()) {
    throw new BadRequestError('Thiếu AI_SECRET_ENCRYPTION_KEY nên không thể lưu khóa API AI')
  }
...
export function decryptSecret(value: string | null | undefined) {
  ...
  if (version !== SECRET_VERSION || !ivRaw || !tagRaw || !encryptedRaw) {
    throw new BadRequestError('Khóa API AI đã lưu không đúng định dạng mã hóa')
  }
