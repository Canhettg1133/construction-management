...
describe("Daily Reports — A3 Daily Report (UAT)", () => {
  ...
  it("AC-2.1 VIEWER cannot create report", async () => {
    const token = signToken("VIEWER");
    const res = await request(app)
  ...
  it("AC-2.1 payload validated before business logic (missing required fields)", async () => {
  ...
  it("DELETE /reports/:reportId requires ADMIN or PROJECT_MANAGER", async () => {
