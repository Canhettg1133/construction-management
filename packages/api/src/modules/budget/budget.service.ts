...
function parseMoney(value: unknown, fieldName: string): Prisma.Decimal {
  ...
  if (amount.lt(0)) {
    throw new BadRequestError(`${fieldName} phải >= 0`)
  }
...
function parseBudgetStatus(value: unknown): 'PENDING' | 'APPROVED' | 'PAID' {
  ...
  if (status !== 'PENDING' && status !== 'APPROVED' && status !== 'PAID') {
    throw new BadRequestError('Trạng thái ngân sách không hợp lệ')
  }
  return status
}
...
function parseDisbursementStatus(value: unknown): 'APPROVED' | 'PAID' {
  ...
  if (status !== 'APPROVED' && status !== 'PAID') {
    throw new BadRequestError('Trạng thái duyệt giải ngân không hợp lệ')
  }
  return status
}
...
async function ensureBudgetItemInProject(projectId: string, itemId: string) {
  ...
  if (!item) {
    throw new NotFoundError('Không tìm thấy hạng mục ngân sách')
  }
...
export const budgetService = {
    ...
    const summary = items.reduce(
      (acc, item) => {
        ...
        const pendingDisbursementAmount = item.disbursements
          ...
          .reduce((total, disbursement) => total.plus(new Prisma.Decimal(disbursement.amount)), new Prisma.Decimal(0))
  ...
  async createItem(projectId: string, _actor: BudgetActorContext, payload: Record<string, unknown>) {
    ...
    const category = parseRequiredText(payload.category, 'Danh mục', 100)
    const description = parseRequiredText(payload.description, 'Mô tả', 500)
    const estimatedCost = parsePositiveMoney(payload.estimatedCost, 'Dự toán')
    const approvedCost =
      payload.approvedCost !== undefined && payload.approvedCost !== null
        ? parsePositiveMoney(payload.approvedCost, 'Mức phê duyệt')
        : null

    if (approvedCost && approvedCost.lt(estimatedCost)) {
      throw new BadRequestError('Mức phê duyệt không được nhỏ hơn dự toán')
    }
  ...
  async updateItem(projectId: string, itemId: string, _actor: BudgetActorContext, payload: Record<string, unknown>) {
    await ensureProjectExists(projectId)
    ...
    if (payload.category !== undefined) {
      data.category = parseRequiredText(payload.category, 'Danh mục', 100)
    }

    if (payload.description !== undefined) {
      data.description = parseRequiredText(payload.description, 'Mô tả', 500)
    }

    if (payload.estimatedCost !== undefined) {
      data.estimatedCost = parsePositiveMoney(payload.estimatedCost, 'Dự toán')
    }
    ...
    if (payload.approvedCost !== undefined) {
      if (payload.approvedCost === null || payload.approvedCost === '') {
        ...
        data.approvedCost = parsePositiveMoney(payload.approvedCost, 'Mức phê duyệt')
      }
    }
  ...
  async createDisbursement(projectId: string, _actor: BudgetActorContext, payload: Record<string, unknown>) {
    await ensureProjectExists(projectId)

    const budgetItemId = parseRequiredText(payload.budgetItemId, 'BudgetItemId', 64)
    const amount = parsePositiveMoney(payload.amount, 'Số tiền giải ngân')
    const note = parseOptionalText(payload.note, 'Ghi chú', 2000)
    ...
    if (spent.plus(amount).gt(limit)) {
      throw new BadRequestError('Số tiền giải ngân vượt quá ngân sách được phê duyệt')
    }
  ...
  async approveDisbursement(
    ...
    if (!disbursement) {
      throw new NotFoundError('Không tìm thấy phiếu giải ngân')
    }

    if (disbursement.status !== 'PENDING') {
      throw new BadRequestError('Phiếu giải ngân đã được xử lý trước đó')
    }
