...
export const warehouseController = {
  ...
  async getInventoryItem(req: Request, res: Response) {
    const data = await warehouseService.getInventoryItem(readProjectId(req), String(req.params.id), readActor(req))
    return sendSuccess(res, data)
  },

  async createInventoryItem(req: Request, res: Response) {
    const data = await warehouseService.createInventoryItem(readProjectId(req), readActor(req), req.body ?? {})
    res.status(201)
    return sendSuccess(res, data)
  },
