...
router.post(
  ...
  asyncHandler(warehouseController.createInventoryItem),
)
router.patch(
  '/inventory/:id',
  requireToolPermission('WAREHOUSE', 'STANDARD'),
  asyncHandler(warehouseController.updateInventoryItem),
)
router.delete(
  '/inventory/:id',
  requireToolPermission('WAREHOUSE', 'ADMIN'),
  asyncHandler(warehouseController.deleteInventoryItem),
)
