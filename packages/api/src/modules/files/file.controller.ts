...
export const fileController = {
  ...
  async view(req: Request, res: Response) {
    const file = await fileService.getById(String(req.params.projectId), String(req.params.fileId))
    const fullPath = resolveStoredFilePath(file.filePath, file.fileName)

    if (!fullPath) {
      throw new NotFoundError('Tệp không tồn tại trên ổ đĩa')
    }
  ...
  async delete(req: Request, res: Response) {
    await fileService.delete(String(req.params.projectId), String(req.params.fileId), req.user?.id)
