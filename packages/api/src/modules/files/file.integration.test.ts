...
describe('Files - request contract', () => {
  ...
  it('GET /projects/:projectId/files returns list for authenticated user', async () => {
    const res = await request(app).get(`/api/v1/projects/${PROJECT_ID}/files`).set('Cookie', buildAuthCookie('VIEWER'))
  ...
  it('VIEWER cannot upload files', async () => {
    const res = await request(app).post(`/api/v1/projects/${PROJECT_ID}/files`).set('Cookie', buildAuthCookie('VIEWER'))
